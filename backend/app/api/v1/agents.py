...
def link_agent_document(agent_id: str, document_id: str, session: SessionDep) -> AgentDocumentsResponse:
...
def unlink_agent_document(agent_id: str, document_id: str, session: SessionDep) -> AgentDocumentsResponse:
    return agent_controller.unlink_document_from_agent(session, agent_id, document_id)


@router.post("/{agent_id}/documents/upload", response_model=KnowledgeDocumentUploadResponse, status_code=201)
